...
import org.processmining.analysis.ltlchecker.*;
...
// METHODS
	...
	// Quantifications:
	    ...
	    case SimpleNode.FORALL : {
		    ...
		    HashSet ts = sets.getSet( attr, var.getScope() );
			...
			// forall node.
			SetValueNode svn = new SetValueNode( 
				SetValueNode.VALUE );
			svn.setValue( new SetAttribute(
			    (String) i.next(),
			    Attribute.LITERAL,
			    var
			    )			    
			...
			node.addChild( (FormulaNode) build(
				qNode.jjtGetChild( 1 ), s, node) );
		    };
	...
	// Formula calls
	    ...
	    case SimpleNode.USEFORMULA : {
		    ...
		    ArrayList params = parser.getParameters( 
			    useNode.getName() );
			...
			// valuenode of the substitution.
			    s.add( ((Attribute) j.next()).getValue(), substs.get( attr.getValue() ) );
			} else {
				...
				case Attribute.NUMBER: {
				    ...
				    nvn.setValue( (NumberAttribute) attr );
				    nvn.setBinder( binder );
				    s.add( ( (Attribute) j.next()).getValue(), nvn );
				    }; break;
...
private TreeNode compOp( Node pNode, Substitutes substs, 
    ...
    String attrName = attribute.getAttributeId();
	...
	case Attribute.SET : {
	    ...
	    SetAttribute sat = (SetAttribute) attribute;
